    ...
    def save(self):
        ...
        for ent in self.classifier:
            ...
            for k,v in oriDic:
                ...
                myDic[k] = v
            cates_info["dict"] = myDic
            cates_info["pd"] = ent.cates_info[0][1]
            cates_info["total"] = ent.cates_info[0][2]

            json_part["cates_info"] = cates_info
            parsed_json = json.dumps(json_part, indent=4, sort_keys=True, ensure_ascii=False)
            print("Create folder if not exist")
            os.mkdir(self.name)
            file = io.open(self.name + "/model_" + self.name + "_" + str(i) + ".json", "w", encoding='utf-8')
